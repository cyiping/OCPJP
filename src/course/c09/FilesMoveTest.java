...
public class FilesMoveTest {
	...
	private static void testMove() throws IOException {
		...
		Path pFromBasic = Paths.get("dir/c09/from");
		Path pTo = Paths.get("dir/c09/moveTo/");
		...
		Path movedFile = Files.move(pFromBasic.resolve(target), Paths.get("dir/c09/moveTo1/"));
		...
		try {
			Path mv = Files.move(pFromBasic.resolve(target), Paths.get("dir/c09/moveTo2/"));	// no REPLACE_EXISTING
			//java.nio.file.FileAlreadyExistsException: dir\c09\moveTo2
			Files.delete(mv);
		} catch (java.nio.file.FileAlreadyExistsException e) {
			e.printStackTrace();
		}
		try {
			Path mv = Files.move(pFromBasic.resolve(target), Paths.get("dir/c09/moveTo2/"), StandardCopyOption.REPLACE_EXISTING);
			Files.delete(mv);
			Files.createDirectories(Paths.get("dir/c09/moveTo2/"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			Path mv = Files.move(pFromBasic.resolve(target), Paths.get("dir/c09/moveTo3/"), StandardCopyOption.REPLACE_EXISTING);
			//java.nio.file.DirectoryNotEmptyException: dir\c09\moveTo3
			Files.delete(mv);
...
//		clean(pTo);
		...
		target = "dirFrom2";
		Path p3 = Files.move(pFromBasic.resolve(target), Paths.get("dir/c09/moveTo1"));
		Files.delete(p3);
		...
		target = "dirFrom3";
		try {
			Path p4 = Files.move(pFromBasic.resolve(target), Paths.get("dir/c09/moveTo2"));	// no REPLACE_EXISTING
			Files.delete(p4);
		} catch (java.nio.file.FileAlreadyExistsException e) {
			e.printStackTrace();
		}
		Path p5 = Files.move(pFromBasic.resolve(target), Paths.get("dir/c09/moveTo2"), StandardCopyOption.REPLACE_EXISTING);
		Files.delete(p5);
		Files.createDirectories(Paths.get("dir/c09/moveTo2/"));
	...
	private static void init(String type) throws IOException {
		...
		Path pFrom = Paths.get("dir/c09/from").toAbsolutePath();
		Files.walkFileTree(pFrom, new FileRemoveVisitor());
		Files.createDirectories(pFrom);
		
		Path pTo = Paths.get("dir/c09/" + type + "/").toAbsolutePath();
		Files.walkFileTree(pTo, new FileRemoveVisitor());
		Files.createDirectories(pTo);
		
		Path pFromOri = Paths.get("dir/c09/from_ori").toAbsolutePath();
